...
class BoVW:
    ...
    def get_feature_vectors(self, extracted_features: pd.DataFrame) -> tuple:
        """
        Calculates the histogram and metadata for each image in a set of extracted features.

        Args:
            extracted_features (pd.DataFrame): A DataFrame representing the extracted features.

        Returns:
            tuple: A tuple containing the histogram DataFrame and the metadata DataFrame.
        """
        histogram_columns = [
        ...
        for _, group in extracted_features.groupby('image_path'):
            ...
            metadata = group[self.metadata_columns].iloc[0].tolist()

            histogram_list.append([group['image_path'].iloc[0]] + histogram)
            metadata_list.extend([metadata + [idx] for idx in cluster_idx])

        histogram_columns = ['image_path'] + histogram_columns
        histogram_dataframe = pd.DataFrame(histogram_list, columns=histogram_columns)
        metadata_columns = self.metadata_columns + ['cluster_idx']
